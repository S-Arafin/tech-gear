...
import { FaTrash, FaMinus, FaPlus, FaArrowLeft, FaShieldAlt, FaCreditCard } from 'react-icons/fa';
    ...
    return (
        <div className="min-h-screen bg-base-200 flex flex-col items-center justify-center p-4">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
                ...
                <p className="text-base-content/60 mb-8">Looks like you haven't added any tech gear yet.</p>
                <Link href="/items" className="btn btn-primary btn-wide">Start Shopping</Link>
            </motion.div>
        </div>
    );
  }
    ...
    <div className="min-h-screen bg-base-200 py-12 px-4">
      <div className="container mx-auto max-w-6xl">
        <h1 className="text-3xl font-bold mb-8 flex items-center gap-2">
            Shopping Cart <span className="text-base-content/40 text-lg font-normal">({cart.length} items)</span>
        </h1>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-base-content/50">${item.price} each</div>
                                    </div>
                                </div>
                                
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-3 bg-base-200 rounded-lg p-1">
                                        <button onClick={() => updateQuantity(item._id, -1)} className="btn btn-sm btn-ghost btn-square" disabled={item.qty <= 1}><FaMinus size={10} /></button>
                                        <span className="font-bold w-4 text-center">{item.qty}</span>
                                        <button onClick={() => updateQuantity(item._id, 1)} className="btn btn-sm btn-ghost btn-square"><FaPlus size={10} /></button>
                                    </div>
                                    
                                    <button onClick={() => removeFromCart(item._id)} className="btn btn-ghost btn-sm text-error gap-2">
                ...
                <div className="flex justify-between items-center mt-6">
                    ...
                    <button onClick={clearCart} className="btn btn-outline btn-error btn-sm">Clear Cart</button>
                </div>
            </div>
